...
In addition to the global limits above, each endpoint has a maximum call frequency based on its data type. Exceeding the per-endpoint limit returns a `429` response even if the global budget has capacity.

#### Static / Slow-Changing Data

| Endpoint | Rate Limit | Rationale |
|----------|-----------|-----------|
| `GET /sports` | 60 req/hour | Sport list rarely changes |
| `GET /leagues` | 60 req/hour | League list rarely changes |
| `GET /matches/{id}/lineups` | 60 req/hour | Set before kickoff, does not change mid-match |
| `GET /matches/{id}/insights` | 60 req/hour | Pre-match data |

#### Semi-Real-Time Data

| Endpoint | Rate Limit | Rationale |
|----------|-----------|-----------|
| `GET /matches` | 60 req/min | Match list and status updates |
| `GET /matches/{id}` | 60 req/min | Match detail polling |
| `GET /matches/{id}/statistics` | 60 req/min | Stats update periodically during a match |
| `GET /matches/{id}/streams` | 12 req/min | Stream URLs; re-fetch only on playback start or failure |

#### Real-Time Data

| Endpoint | Rate Limit | Rationale |
|----------|-----------|-----------|
| WebSocket live events | No REST limit — use WebSocket | Pushed in real-time via subscription |

<Tip>
For real-time data (live scores, incidents, odds), always prefer **WebSocket** over REST polling. WebSocket subscriptions are not subject to REST rate limits and deliver updates with lower latency.
</Tip>
...
If you receive a `429 Too Many Requests` response, you must pause sending requests until the rate limit window resets.
...
- **`Retry-After`**: Seconds to wait before retrying (preferred for simple retry logic)
- **`X-RateLimit-Reset`**: UNIX timestamp when the window resets (preferred for scheduling)
